...
import "../../harvester/IExchangeRouter.sol";
...
contract FTokenStrat is IStrat {
    ...
    string public name = "Vault Staking"; // for display purposes only

    event Rugested(uint block, uint amount);
    ...
    constructor(IVault vault_, IFToken fToken_, IRewardPool rewardpool_, IExchangeRouter router_) {
    ...
    function invest() external override onlyVault {
        ...
        if(balance > buffer) {
            ...
            emit Rugested(block.number, balance - buffer);
        }
    }
